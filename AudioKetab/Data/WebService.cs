...
namespace AudioKetab
{
    public static class WebService
    {
        ...
        public static List<NotificatonModel.OtherNotification> GetAllNotificaton()
        {

            string ret = string.Empty;
            List<NotificatonModel.OtherNotification> listNotification = null;
            try
            {
                HttpResponseMessage response = null;
                JObject j = new JObject();
                j.Add("method", "get_notifications");
                j.Add("user_id", StaticDataModel.UserId);
                ...
                if (response.IsSuccessStatusCode)
                {
                    using (StreamReader reader = new StreamReader(response.Content.ReadAsStreamAsync().Result))
                    {
                        ...
                        if (ret == "success")
                        {
                           listNotification = DataValues["other_notification"].ToObject<List<NotificatonModel.OtherNotification>>();
                        }
                    }
